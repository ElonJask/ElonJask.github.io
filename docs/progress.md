...
- 已完成：首页搜索功能回归（中英双语）：
  - 新增首页搜索 UI 与结果列表，支持按标题/摘要/正文片段/标签/分类检索；
  - 搜索数据源复用 `search.json`，按语言自动过滤；
  - 空查询自动恢复默认文章列表与分页，避免破坏现有浏览路径；
  - 支持快捷键 `/` 聚焦搜索框、`Esc` 清空搜索。
- 已完成：首页列表排版协调优化：
  - 桌面端文章标题/摘要字号上调（标题 `24px`，摘要 `17px`）；
  - 移动端文章标题/摘要字号同步抬升并细分断点，提升可读性与层级一致性。
- 已完成：新增首页搜索脚本 `theme/home-search.js`（仅首页加载），并保持 `bundle exec jekyll build` 构建通过。
