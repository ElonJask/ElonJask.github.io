...
- 已完成：首页搜索功能回归（中英双语）：
- 已完成：首页搜索方案调整为“导航图标 + 弹窗搜索”：
  - 导航右侧新增搜索图标按钮，点击后打开搜索弹窗；
  - 搜索支持标题/摘要/正文片段/标签/分类检索，数据源复用 `search.json`；
  - 弹窗支持 `/` 打开、`Esc` 关闭、`↑↓` 选择、`Enter` 跳转；
  - 搜索结果按当前语言过滤（中文页仅中文、英文页仅英文）。
- 已完成：移除语言切换图标，统一改为文字切换（`EN/中`），提升导航在移动端与桌面端的一致性与协调度。
- 已完成：按反馈将“搜索图标 + 语言文字”调为协调版（非紧缩版）：
  - 恢复与导航同级的字号/触达面积（桌面 `34px` 按钮、移动端分档缩放）；
  - 调整搜索与语言的间距，避免过挤导致视觉突兀。
- 已完成：回归检查（本地 `http://127.0.0.1:4001`）：
  - `bundle exec jekyll build` 通过；
  - Lighthouse mobile（首页/专题/文章）Performance 均为 `100`。
- 已完成：首页列表排版协调优化：
  - 桌面端文章标题/摘要字号上调（标题 `24px`，摘要 `17px`）；
  - 移动端文章标题/摘要字号同步抬升并细分断点，提升可读性与层级一致性。
- 已完成：新增搜索弹窗脚本 `theme/search-modal.js` 与模板 `_includes/search.html`，并保持 `bundle exec jekyll build` 构建通过。
- 已完成：专题页标题文案简化，`专题分类` 调整为 `专题`（`/topics` 页面标题与导航文案保持一致）。
